...
func (rt *_router) getFollowed(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	var followed []database.Followed
	// get list of followed users from database
	followed, err = rt.db.GetFollowed(pathusername)
	if len(followed) == 0 {
		http.Error(w, "User is not following anyone", http.StatusNotFound)
		return
	}
	if err != nil {
		http.Error(w, "Internal server error", http.StatusInternalServerError)
	...
	jsonResponse, err := json.Marshal(followed)
	if err != nil {
